...
public class GenUtils {
	...
	public static void generatorCode(Map<String, String> table,
        ...
        tableDO.setTableComment(table.get("tableComment"));
        //表名转换成Java类名
        ...
        map.put("comments", tableDO.getTableComment());
    ...
    @SuppressWarnings("deprecation")
   	public static void generatorCode(TableDO table,
           ...
           Configuration config = getConfig();
           //表名转换成Java类名
           String className = tableToJava(table.getTableName(), config.getString("tablePrefix"), config.getString("autoRemovePre"));
           ...
           for (String template : templates) {
               ...
               try {
                   //添加到zip
                   zip.putNextEntry(new ZipEntry(getFileName(template, table.getClassname(), table.getClassName(), config.getString("package").substring(config.getString("package").lastIndexOf(".") + 1))));
                   IOUtils.write(sw.toString(), zip, "UTF-8");
                   IOUtils.closeQuietly(sw);
                   zip.closeEntry();
               } catch (IOException e) {
                   throw new BDException("渲染模板失败，表名：" + table.getTableName(), e);
               }
           }
       }
    /**
